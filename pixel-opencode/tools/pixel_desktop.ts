...
export default tool({
  description: 'Use the full Debian/XFCE desktop, including OrcaSlicer. Start only when needed; poll status until ready. Screenshot returns an image path to read. Mouse coordinates are desktop pixels. launch runs a Linux command with DISPLAY=:1. Use the normal bash tool for files, coding and packages. show opens the desktop on the phone; hide returns to Pixel Agent without closing Linux apps. The return arrow also returns to chat. Desktop input resets idle sleep. Save documents before leaving. stop closes only a managed desktop and its apps.',
  args: {
    action: tool.schema.enum(['start','status','stop','show','hide','screenshot','launch','click','move','drag','scroll','key','type']),
